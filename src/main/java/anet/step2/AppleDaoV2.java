...
public class AppleDaoV2 {
     ...
     * @param dong // 사용자가 입력한 동 이름
     * @return ZipCodeVO[]
     *************************************************/
    public ZipCodeVO[] getAddress(String zdo, String dong){
        StringBuilder sql = new StringBuilder();
        sql.append("SELECT zipcode, address");//부적합한 식별잘 - Toad
        ...
        try {
            ...
            if (dong != null && dong.length() > 0) {
                pstmt.setString(i++, dong);//JTextField에 입력 후 엔터 쳤을 때 혹은 찾기 버튼 눌렀을 때
            }
            //조회나 상세조회일 때는 executeQuery를 사용한다.
            //executeQuery는 리턴타입이 ResultSet이고,
            //입력, 수정, 삭제 일때는 executeUpdate를 사용한다.
            //executeUpdate는 리턴타입 int이다.
    ...
    public String[] getZdoList(){
        ...
        try {
            ...
            System.out.println(zdos.length);//0이 출력됨
            v.copyInto(zdos);
