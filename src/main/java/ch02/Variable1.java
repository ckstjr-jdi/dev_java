...
public class Variable1 {
        ...
        //컴파일을 위해서 JVM을 설치함
        //컴파일 과정에서 타입을 체크한다. - 타입이 틀리면 문법에러가 발생함. - 실행 불가함.
        //문법 오류가 없어야 정상적으로 XXX.class생성됨 -> 실제로 이 파일이 실행됨.
        //운영서버에 올리는 파일도 XXX.class파일이다.
        //보안 이슈로 패키지(폴더)가 없는 클래스는 에러임
        //실행파일로 만들기 위해서는 main메서드가 있어야 한다. -> 웹기반이 아니다.
        //로컬 에서 실행할 때 필요한 메서드이다.
        //가정 먼저 실행되는 것이 main메소드 입니다. - entry point - main thread
        public static void main(String[] args) { //14-15
            System.out.println("main 호출");
            // 원시형 타입의 변수 선언하기(8가지)
            int empno = 7566; //변수 선언 및 초기화
            empno = 7499;
            int deptno; //변수 선언
            deptno = 10; //초기화
