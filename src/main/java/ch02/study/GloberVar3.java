...
import javax.swing.*;
// main메서드느 엔트리 포인트 입니다. - main thread -> 가장 먼저 호출 된다.
// main메서드가 있어야 exe파일이 생성됨 -> c++ dll, exe
// main메서드는 개발자가 호출하는 메서드가 아니다. -> 그래서 main메서드 코딩을 적게 해야 함.
// main메서드 밖에 구현한 것들이 자유도가 높다.
// 전역변수 isView를 사용하여 윈도우 창이 화면에 출력되도록 코드를 작성하시오.
public class GloberVar3 {
    boolean isview = false;
    //파라미터 자리도 변수를 선언하는 자리이다.
    //초기화가 안되어 있다.- 원본이어야 한다.
    //methodA를 호출하기 전에 isview를 true로 변경해 주는
