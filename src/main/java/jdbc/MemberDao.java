...
public class MemberDao {
    ...
    Connection              con    = null; //물리적으로 떨어져 있는 오라클서버에 세션을 연결함
    PreparedStatement       pstmt  = null; //개발자가 작성한 select문 전달하고 요청함
    ResultSet               rs     = null; //오라클의 cursor를 추상화한 인터페이스임
    /****************************************************************************************
    * 오라클 soctt계정에서 회원정보 가져오기 구현
    * SELECT mem_id, mem_pw, mem_name, mem_nickname, mem_age
    *  FROM SCOTT.member
    * WHERE mem_id = 'kiwi'
    * @param mem_id
    * @return MemverVO[] - 객체배열
     * 나는 메서드를 설계할 수 있다.
    ...
    public MemberVO[] getMemberList(String mem_id){
        ...
        try {
            ...
            //2. 오라클 서버와 연결 객체 생성
            // JDBC API -> MyBais -> JPA(Hibernate)
            con = DriverManager.getConnection(_URL, _USER, _PW);
            //3. 연결객체를 통해서 쿼리문을 전달하고 오라클에게 select문 처리 요청함.
            pstmt = con.prepareStatement(sql.toString());
            //4. select문을 요청하면 오라클은 open..cursor..fetch과정을 거침
            //rs.next():커서를 다음으로 rs.previous():커서를 이전으로 rs.absolute(3):커서를 세번째로
            rs = pstmt.executeQuery();
            System.out.println(rs.next());//ture이면 레코드가 있음, false반환하면 레코드 없음.
            ...
            //그래서 백터안에 있는 데이터를 배열에 복사해주는 메서드를 찾았다.
            al.copyInto(mvos);
            for(int i=0;i<mvos.length;i++){
                MemberVO rmvo = mvos[i];
                System.out.println(rmvo.getMem_id()+", "+rmvo.getMem_age()+", "+rmvo.getMem_name()+", "+rmvo.getMem_nickname()+", "+rmvo.getMem_pw());
            }
